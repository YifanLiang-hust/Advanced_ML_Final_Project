...
class LoCoOp(TrainerX):
    ...
    def test_ood(self, data_loader, T):
        ...
        for batch_idx, (images, labels, *id_flag) in enumerate(tqdm(data_loader)):
            ...
            smax_local = to_np(output_local)
            smax_global = to_np(F.softmax(output/T, dim=-1))
            smax_local = to_np(F.softmax(output_local/T, dim=-1))
            smax_local_max = np.max(smax_local, axis=1)
            mcm_pred = np.argmax(smax_global, axis=1)
            glmcm_pred = np.argmax(smax_global+smax_local_max, axis=1)
            mcm_global_score = np.max(smax_global, axis=1)
            mcm_local_score = np.max(smax_local, axis=(1, 2))
            mcm_pred_list.append(mcm_pred)
            glmcm_pred_list.append(glmcm_pred)
            mcm_score.append(mcm_global_score)
            factor = 1
            glmcm_score.append(mcm_global_score + factor*mcm_local_score)

        return mcm_pred_list, glmcm_pred_list, concat(mcm_score)[:len(data_loader.dataset)].copy(), concat(glmcm_score)[:len(data_loader.dataset)].copy()
