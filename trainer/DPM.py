...
class DPM(TrainerX):  # need to be trained
    ...
    #     dpm_best_scores = {fea: {ood_name: {'beta': None, 'aur': -float('inf'), 'fpr': float('inf')} for ood_name in out_datasets} for fea in ['fea1_mls', 'fea2_mls', 'fea3_mls', 'fea1_mcm', 'fea2_mcm', 'fea3_mcm']}
    ...
    #     dpm_global_best_scores = {fea: {'beta': None, 'aur': -float('inf'), 'fpr': float('inf')} for fea in ['fea1_mls', 'fea2_mls', 'fea3_mls', 'fea1_mcm', 'fea2_mcm', 'fea3_mcm']}
    ...
    #         # 固定 alpha=1，遍历 beta
    #         alpha = 1.0
    #         for beta in tqdm(np.arange(-10.0, 0.1, 0.1), desc="Beta Loop", leave=False):
    ...
    #                 # 记录dpm性能
    #                 dpm_global_scores[fea][ood_name].append({'beta': beta, 'aur': aur, 'fpr': fpr})
    #                 if score > dpm_best_scores[fea][ood_name].get('aur', -float('inf')) - dpm_best_scores[fea][ood_name].get('fpr', float('inf')):
    #                     dpm_best_scores[fea][ood_name] = {'beta': beta, 'aur': aur, 'fpr': fpr}
            
    #         # 单独计算并记录 TM (alpha=1, beta=0) 和 VM (alpha=0, beta=-1)
    #         for fea, kl_id_norm, kl_ood_norm, in_fea, out_fea in [
    #             ('fea1_mls', kl_id_norm_mls1, kl_ood_norm_mls1, in_fea1_mls, out_fea1_mls),
    #             ('fea2_mls', kl_id_norm_mls2, kl_ood_norm_mls2, in_fea2_mls, out_fea2_mls),
    #             ('fea3_mls', kl_id_norm_mls3, kl_ood_norm_mls3, in_fea3_mls, out_fea3_mls),
    ...
    #             ('fea3_mcm', kl_id_norm_mcm3, kl_ood_norm_mcm3, in_fea3_mcm, out_fea3_mcm)
    #         ]:
    #             # 计算 TM (alpha=1, beta=0)
    #             tm_aur, tm_fpr, _ = compute_scores(1.0, 0.0, kl_id_norm, kl_ood_norm, in_fea, out_fea)
    #             tm_scores[fea][ood_name] = {'aur': tm_aur, 'fpr': tm_fpr}
                
    #             # 计算 VM (alpha=0, beta=-1)
    #             vm_aur, vm_fpr, _ = compute_scores(0.0, -1.0, kl_id_norm, kl_ood_norm, in_fea, out_fea)
    #             vm_scores[fea][ood_name] = {'aur': vm_aur, 'fpr': vm_fpr}
        
    #     # 遍历所有特征
    #     for fea in ['fea1_mls', 'fea2_mls', 'fea3_mls', 'fea1_mcm', 'fea2_mcm', 'fea3_mcm']:
    #         # 遍历所有beta值
    #         for beta in np.arange(-10.0, 0.1, 0.1):
    #             beta = round(beta, 1)  # 确保精度
                
    #             # 筛选符合条件的记录
    #             filtered_records = []
    #             for ood_name in out_datasets:
    #                 for record in dpm_global_scores[fea][ood_name]:
    #                     if record['beta'] == beta:
    #                         filtered_records.append(record)
    #                         break

    #             # 如果没有符合条件的记录，跳过当前beta值
    ...
    #                 dpm_global_best_scores[fea] = {'beta': beta, 'aur': avg_aur, 'fpr': avg_fpr}
    ...
    #             fixed_record = None
    #             for record in dpm_global_scores[fea][ood_name]:
    #                 if record['beta'] == best_beta:
    #                     fixed_record = record
    #                     break
                
    #             if fixed_record:
    #                 fixed_aur = fixed_record['aur']
    #                 fixed_fpr = fixed_record['fpr']
    ...
    #             print(f"\nFeature: {fea}")
    #             # 打印 TM 和 VM 的性能
    #             print(f"TM (alpha=1, beta=0): AUR = {tm_scores[fea][ood_name]['aur']:.4f}, FPR = {tm_scores[fea][ood_name]['fpr']:.4f}")
    #             print(f"VM (alpha=0, beta=-1): AUR = {vm_scores[fea][ood_name]['aur']:.4f}, FPR = {vm_scores[fea][ood_name]['fpr']:.4f}")
                
    #             # 打印 dpm 的 Unfixed 和 Fixed 性能
    #             print(f"dpm (Unfixed): beta = {dpm_best_scores[fea][ood_name]['beta']:.1f}, AUR = {dpm_best_scores[fea][ood_name]['aur']:.4f}, FPR = {dpm_best_scores[fea][ood_name]['fpr']:.4f}")
    #             if 'fixed' in dpm_best_scores[fea][ood_name]:
    #                 print(f"dpm (Fixed): beta = {dpm_global_best_scores[fea]['beta']:.1f}, AUR = {dpm_best_scores[fea][ood_name]['fixed']['aur']:.4f}, FPR = {dpm_best_scores[fea][ood_name]['fixed']['fpr']:.4f}")
    ...
    #         print(f"TM (alpha=1, beta=0, Average): AUR = {avg_tm_aur:.4f}, FPR = {avg_tm_fpr:.4f}")
    #         print(f"VM (alpha=0, beta=-1, Average): AUR = {avg_vm_aur:.4f}, FPR = {avg_vm_fpr:.4f}")
    ...
    #         # 打印 dpm 的 Fixed 性能
    #         print(f"dpm (Fixed): beta = {dpm_global_best_scores[fea]['beta']:.1f}, AUR = {dpm_global_best_scores[fea]['aur']:.4f}, FPR = {dpm_global_best_scores[fea]['fpr']:.4f}")
    ...
    def test_ood(self, train_loader, id_loader, ood_loader_list, out_datasets, t=100):
        ...
        # 获取 ID 特征
        in_fea2_mls, in_kl2 = self.get_feature(class_sim_mean2, id_loader)
        in_fea2_mcm = np.array([softmax(item / t) for item in in_fea2_mls])
        ...
        # 初始化统计变量
        tm_scores = {fea: {ood_name: {} for ood_name in out_datasets} for fea in ['fea2_mls', 'fea2_mcm']}
        vm_scores = {fea: {ood_name: {} for ood_name in out_datasets} for fea in ['fea2_mls', 'fea2_mcm']}
        dpm_best_scores = {fea: {ood_name: {'beta': None, 'aur': -float('inf'), 'fpr': float('inf')} for ood_name in out_datasets} for fea in ['fea2_mls','fea2_mcm']}
        dpm_global_scores = {fea: {ood_name: [] for ood_name in out_datasets} for fea in ['fea2_mls', 'fea2_mcm']}
        dpm_global_best_scores = {fea: {'beta': None, 'aur': -float('inf'), 'fpr': float('inf')} for fea in ['fea2_mls', 'fea2_mcm']}

        # 遍历 OOD 数据集
        for i, ood_name in enumerate(out_datasets):
            print(f'******* {ood_name} *******')
            ood_loader = ood_loader_list[i]
            out_fea2_mls, out_kl2 = self.get_feature(class_sim_mean2, ood_loader)
            out_fea2_mcm = np.array([softmax(item / t) for item in out_fea2_mls])
        ...
        for fea in ['fea2_mls', 'fea2_mcm']:
            ...
            for beta in np.arange(-10.0, 0.1, 0.1):
                ...
                for ood_name in out_datasets:
                    for record in dpm_global_scores[fea][ood_name]:
                        if record['beta'] == beta:
                            all_aurs.append(record['aur'])
                            all_fprs.append(record['fpr'])
                            break
                
                # 如果所有数据集都有该beta值的记录
                if len(all_aurs) == len(out_datasets):
                    avg_aur = np.mean(all_aurs)
    ...
    def get_feature(self, class_sim_mean2, loader):
        # total_logit1 = []
        total_logit2 = []
        # total_logit3 = []
        # kl_div1 = []
        ...
        return total_logit2.detach().cpu().numpy(), kl_div2

    def draw_tsne_multi(self, id_loader, cs_id_loader, ood_loader_list, out_datasets, 
                output_dir='./tsne_multi_results', perplexity=30, n_iter=1000, 
                random_state=1555, feature_type='feat_v'):
        """
        为多类型数据集创建t-SNE可视化: ID数据、协变量偏移ID数据和OOD数据
        
        参数:
            id_loader: 标准ID数据的DataLoader
            cs_id_loader: 协变量偏移ID数据的DataLoader
            ...
            n_iter: t-SNE的迭代次数
            random_state: 随机种子
            feature_type: 要可视化的特征类型，可选值: 'feat_v', 'clip_feature'
        """
        import os
        import numpy as np
        import matplotlib.pyplot as plt
        ...
        # 为OOD数据集分配对比色
        ood_colors = ['#2ecc71', '#9b59b6', '#f1c40f', '#1abc9c', '#34495e']
        ...
        # 1. 收集ID数据特征
        print(f"获取标准ID数据的{feature_type}特征...")
        id_features = []
        with torch.no_grad():
            for batch_idx, (images, _) in tqdm(enumerate(id_loader)):
                images = images.cuda()
                Fv, Fs, feat_v, feat_vv, clip_feature = self.model.get_feature(images)
                if feature_type == 'feat_v':
                    id_features.append(feat_v.mean(dim=1).cpu().numpy())
                else:  # clip_feature
                    id_features.append(clip_feature.cpu().numpy())
        
        all_features['ID'] = np.vstack(id_features) if id_features else np.array([])
        
        # 2. 收集协变量偏移ID数据特征
        print(f"获取协变量偏移ID数据的{feature_type}特征...")
        cs_id_features = []
        with torch.no_grad():
            for batch_idx, (images, _) in tqdm(enumerate(cs_id_loader)):
                images = images.cuda()
                Fv, Fs, feat_v, feat_vv, clip_feature = self.model.get_feature(images)
                if feature_type == 'feat_v':
                    cs_id_features.append(feat_v.mean(dim=1).cpu().numpy())
                else:  # clip_feature
                    cs_id_features.append(clip_feature.cpu().numpy())
        
        all_features['CS-ID'] = np.vstack(cs_id_features) if cs_id_features else np.array([])
        
        # 3. 收集OOD数据特征
        for i, (ood_name, ood_loader) in enumerate(zip(out_datasets, ood_loader_list)):
            print(f"获取OOD数据集 {ood_name} 的{feature_type}特征...")
            ood_features = []
            with torch.no_grad():
                for batch_idx, (images, _) in tqdm(enumerate(ood_loader)):
                    ...
                    Fv, Fs, feat_v, feat_vv, clip_feature = self.model.get_feature(images)
                    if feature_type == 'feat_v':
                        ood_features.append(feat_v.mean(dim=1).cpu().numpy())
                    else:  # clip_feature
                        ood_features.append(clip_feature.cpu().numpy())
            
            all_features[ood_name] = np.vstack(ood_features) if ood_features else np.array([])
        
        # 检查是否有数据
        feature_counts = {k: len(v) for k, v in all_features.items()}
        print("收集到的特征数量:", feature_counts)
        
        # 移除空数据集
        all_features = {k: v for k, v in all_features.items() if len(v) > 0}
        
        if len(all_features) <= 1:
            print("警告: 数据集数量不足，无法创建有意义的t-SNE可视化")
            return
        
        # 合并所有特征
        print("合并所有特征并应用t-SNE降维...")
        combined_features = []
        feature_types = []
        
        for dataset_type, features in all_features.items():
            combined_features.append(features)
            feature_types.extend([dataset_type] * len(features))
        
        combined_features = np.vstack(combined_features)
        
        # 应用t-SNE降维
        print(f"对{len(combined_features)}个数据点应用t-SNE降维...")
        adjusted_perplexity = min(perplexity, len(combined_features) - 1)
        tsne = TSNE(
            n_components=2, 
        ...
        # 按顺序绘制数据点
        for dataset_type in ['ID', 'CS-ID'] + out_datasets:
            if dataset_type in all_features:
                features = all_features[dataset_type]
                end_idx = start_idx + len(features)
                
                plt.scatter(
                    embeddings_2d[start_idx:end_idx, 0],
                    embeddings_2d[start_idx:end_idx, 1],
                    c=colors[dataset_type],
        ...
        # 保存图像
        plt.tight_layout()
        file_path = os.path.join(output_dir, f"tsne_multi_{feature_type}.png")
        plt.savefig(file_path, dpi=300, bbox_inches='tight')
        plt.close()
        
        print(f"多数据集t-SNE可视化已保存至: {file_path}")
        
        return embeddings_2d, feature_types
        ...
        for feat_type in features_dict:
            ...
            if all_ood_feats:
                ...
                plt.scatter(
                    ...
                    s=30, 
                    marker='o',
                    edgecolors='w',           # 白色边框增强视觉分离
                    linewidth=0.5             # 细边框更精致
                )
    ...
    def draw_tsne_combined(self, id_loader, ood_loader_list, out_datasets, output_dir='./tsne_combined', 
        ...
        # 在 draw_tsne_combined 函数中添加，放在绘图完成后、函数返回前的位置

        # 保存原型坐标信息到文本文件
        if 'Textual Prototype' in features and 'Visual Prototype' in features:
            prototype_coords_path = os.path.join(output_dir, "prototype_coordinates.txt")
            class_names = self.dm.dataset.classnames
            n_prototypes = min(len(text_embeddings), len(visual_embeddings))
            
            with open(prototype_coords_path, 'w') as f:
                f.write("Prototype Coordinates in t-SNE 2D Space\n")
                f.write("="*60 + "\n")
                f.write(f"{'Class Index':<12}{'Class Name':<30}{'Type':<15}{'X Coord':<15}{'Y Coord':<15}\n")
                f.write("-"*80 + "\n")
                
                # 记录所有原型的坐标
                for i in range(n_prototypes):
                    text_x, text_y = text_embeddings[i, 0], text_embeddings[i, 1]
                    visual_x, visual_y = visual_embeddings[i, 0], visual_embeddings[i, 1]
                    
                    # 获取类名(类索引小于类名列表长度时)
                    class_name = class_names[i] if i < len(class_names) else f"Unknown-{i}"
                    
                    # 文本原型坐标
                    f.write(f"{i:<12}{class_name:<30}{'Textual':<15}{text_x:<15.6f}{text_y:<15.6f}\n")
                    
                    # 视觉原型坐标
                    f.write(f"{i:<12}{class_name:<30}{'Visual':<15}{visual_x:<15.6f}{visual_y:<15.6f}\n")
                
                # 添加一些统计信息
                f.write("\n\nPrototype Pair Distances in 2D space\n")
                f.write("="*60 + "\n")
                pair_distances = []
                for i in range(n_prototypes):
                    text_coords = text_embeddings[i]
                    visual_coords = visual_embeddings[i]
                    distance = np.sqrt(np.sum((text_coords - visual_coords)**2))
                    pair_distances.append(distance)
                    f.write(f"{i:<12}{class_name:<30}{distance:.6f}\n")
                
                # 添加距离统计
                f.write("\nDistance Statistics:\n")
                f.write(f"Average distance: {np.mean(pair_distances):.6f}\n")
                f.write(f"Maximum distance: {np.max(pair_distances):.6f}\n")
                f.write(f"Minimum distance: {np.min(pair_distances):.6f}\n")
                f.write(f"Standard deviation: {np.std(pair_distances):.6f}\n")
            
            print(f"原型坐标已保存到: {prototype_coords_path}")  

    def plot_ood(self, train_loader, id_loader, ood_loader_list, out_datasets, optimal_params=None, output_dir='./distributions', t=100):
        self.model.eval()
        print('getting ID train cls_sim_mean......')
        total_logit1, total_logit2, total_logit3, labels = [], [], [], []
        ...
        # 获取 ID 特征
        id_fea1_mls, id_fea2_mls, id_fea3_mls, id_kl1, id_kl2, id_kl3 = self.get_feature(class_sim_mean1, class_sim_mean2, class_sim_mean3, id_loader)
        id_fea1_mcm = np.array([softmax(item / t) for item in id_fea1_mls])
        id_fea2_mcm = np.array([softmax(item / t) for item in id_fea2_mls])
        id_fea3_mcm = np.array([softmax(item / t) for item in id_fea3_mls])

        # 遍历 OOD 数据集
        for i, ood_name in enumerate(out_datasets):
            print(f'******* {ood_name} *******')
            ood_loader = ood_loader_list[i]
            ood_fea1_mls, ood_fea2_mls, ood_fea3_mls, ood_kl1, ood_kl2, ood_kl3 = self.get_feature(class_sim_mean1, class_sim_mean2, class_sim_mean3, ood_loader)
            ood_fea1_mcm = np.array([softmax(item / t) for item in ood_fea1_mls])
            ood_fea2_mcm = np.array([softmax(item / t) for item in ood_fea2_mls])
            ood_fea3_mcm = np.array([softmax(item / t) for item in ood_fea3_mls])

            
            print(f"绘制 {ood_name} 的分布图...")
            # 遍历每种特征类型
            for fea_type, params in optimal_params.items():
                alpha = params['alpha']
                beta = params['beta']
                print(f"使用logit: {fea_type}, 使用参数: alpha={alpha}, beta={beta} 进行绘图")

                id_fea, ood_fea, id_kl, ood_kl = {
                    'fea1_mls': (id_fea1_mls, ood_fea1_mls, id_kl1, ood_kl1),
                    'fea2_mls': (id_fea2_mls, ood_fea2_mls, id_kl2, ood_kl2),
                    'fea3_mls': (id_fea3_mls, ood_fea3_mls, id_kl3, ood_kl3),
                    'fea1_mcm': (id_fea1_mcm, ood_fea1_mcm, id_kl1, ood_kl1),
                    'fea2_mcm': (id_fea2_mcm, ood_fea2_mcm, id_kl2, ood_kl2),
                    'fea3_mcm': (id_fea3_mcm, ood_fea3_mcm, id_kl3, ood_kl3),
                }[fea_type]
                ...
                x_max, x_min = max(kl_id_norm.max(), kl_ood_norm.max()), min(kl_id_norm.min(), kl_ood_norm.min())
                target_max, target_min = max(np.max(id_fea), np.max(ood_fea)), min(np.min(id_fea), np.min(ood_fea))
                kl_id_norm_scaled = (kl_id_norm - x_min) / (x_max - x_min) * (target_max - target_min) + target_min
                kl_ood_norm_scaled = (kl_ood_norm - x_min) / (x_max - x_min) * (target_max - target_min) + target_min

                # 计算分数
                id_scores = beta * kl_id_norm_scaled + alpha * np.max(id_fea, axis=1)
                ood_scores = beta * kl_ood_norm_scaled + alpha * np.max(ood_fea, axis=1)

                # 测试指标
                aur, _, fpr = self.get_measures(id_scores, ood_scores)

                # 绘制分布图
                output_dir_fea = os.path.join(output_dir, ood_name)
                os.makedirs(output_dir_fea, exist_ok=True)
                file_name = f"{fea_type}_alpha{alpha}_beta{beta}_aur{aur:.4f}_fpr{fpr:.4f}"
                self._plot_distribution(output_dir_fea, id_scores, ood_scores, file_name, fpr)
